...
async function getWeather() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `City not found or API request failed. Status: ${response.status}`
        );
      }
  ...
  async function getWeatherForCities() {
    ...
    const apiKey = ""; // Add your API key here
    const urlTemplate = `https://api.weatherapi.com/v1/current.json?key=${apiKey}&q=`;
    ...
    for (const city of cities) {
      try {
        ...
        const response = await fetch(
          urlTemplate + encodeURIComponent(city),
          options
        );
        if (!response.ok) {
          console.error(
  ...
  function addWeatherToTable(city, data) {
    ...
    document.getElementById(`${city.toLowerCase()}_temperature`).innerText =
      weather.temp_c;
    document.getElementById(`${city.toLowerCase()}_wind_speed`).innerText =
      weather.wind_kph;
    document.getElementById(`${city.toLowerCase()}_cloud`).innerText =
      weather.cloud;
    document.getElementById(`${city.toLowerCase()}_humidity`).innerText =
      weather.humidity;
    document.getElementById(`${city.toLowerCase()}_heat_index`).innerText =
      weather.feelslike_c;
    document.getElementById(`${city.toLowerCase()}_wind_direction`).innerText =
      weather.wind_dir;
